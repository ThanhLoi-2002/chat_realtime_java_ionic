...
// Định nghĩa sự kiện gửi ra ngoài component cha khi đổi trang
const emit = defineEmits<{
    (e: 'pageChange', newPageIndex: number): void
}>()
...
const tableOptions = computed(() => ({
    ...
    // CẤU HÌNH SERVER-SIDE PAGINATION
    manualPagination: true,
    rowCount: props.totalElements, // Tổng số bản ghi ở database
    ...
    // Khi bấm nút Next/Prev của TanStack Table, hàm này sẽ kích hoạt
    onPaginationChange: (updater: any) => {
        const nextState = typeof updater === "function"
            ? updater({ pageIndex: props.pageIndex, pageSize: props.pageSize ?? 15 })
            : updater;

        // Bắn sự kiện ra ngoài thông báo trang đã thay đổi
        emit('pageChange', nextState.pageIndex);
    },
...
<template>
    ...
    <div class="flex gap-2 mt-4 items-center text-gray-700 dark:text-gray-200 z-1">
        <ion-button @click="table.previousPage()" :disabled="props.pageIndex === 0">
            Prev
        </ion-button>
        ...
        <ion-button @click="table.nextPage()"
            :disabled="(props.pageIndex + 1) * (props.pageSize ?? 15) >= props.totalElements">
            Next
        </ion-button>
    </div>
</template>