...
<template>
    <div class="h-10 flex items-center bg-gray-200 dark:bg-slate-900/40 rounded-b-md select-none relative shrink-0">

        <div class="flex-1 h-full relative flex items-center overflow-hidden group">

            <button @click="handlePrevPack"
                class="absolute left-0 z-10 h-9 w-5 flex items-center justify-center bg-gray-300/80 dark:bg-slate-800/80 text-gray-700 dark:text-gray-300 rounded-r-md cursor-pointer opacity-0 group-hover:opacity-100 transition-opacity backdrop-blur-sm">
            ...
            >
                ...
                <button v-for="pack in packs" :key="pack.stickerId" :id="`pack-btn-${pack.stickerId}`"
                    ...
                    <img :src="STICKER_URL + pack.iconUrl" class="w-full h-full object-contain pointer-events-none" loading="lazy" />
                </button>
            </div>

            <button @click="handleNextPack"
                class="absolute right-0 z-10 h-9 w-5 flex items-center justify-center bg-gray-300/80 dark:bg-slate-800/80 text-gray-700 dark:text-gray-300 rounded-l-md cursor-pointer opacity-0 group-hover:opacity-100 transition-opacity backdrop-blur-sm">
