...
const onSelectItemDelete = (item: ClassificationCardType) => {
    confirmStore.open({
        ...
        onOk: async () => {
            const success = await classificationCardStorage.delete(item.id)
...
<template>
    <div class="flex flex-col h-full bg-white dark:bg-slate-900/50 text-slate-900 dark:text-slate-200 p-4">
        ...
        <div class="flex-1 min-h-0 overflow-y-auto pr-2">
            <draggable v-model="dragList" item-key="id" handle=".drag-handle" :animation="300" tag="div"
                class="space-y-3 pb-4" ghost-class="sortable-placeholder" chosen-class="sortable-chosen"
                drag-class="sortable-drag">
                <template #item="{ element: tag }">
                    <div
                        class="flex items-center group bg-slate-50 dark:bg-[#1c1f24] border border-slate-200 dark:border-slate-800 rounded-xl px-2 py-2 shadow-sm">

                        <div
                            class="drag-handle cursor-grab active:cursor-grabbing p-2 text-slate-400 hover:text-indigo-500 transition-colors">
                            <ion-icon :icon="menuOutline" class="text-2xl" />
                        </div>

                        <div
                            class="flex-1 flex items-center bg-white dark:bg-[#282c33] py-2.5 px-4 rounded-lg shadow-inner-sm ml-1 group-hover:ring-1 group-hover:ring-indigo-500/30 transition-all">
                            ...
                            <div
                                ...
                                <button @click.stop="onSelectItemDelete(tag)"
